...
data=prmdata.apply(pd.to_numeric, errors='coerce') #coerce为字符变为空值
...
pca_var=pd.DataFrame([pca_var2,pca_var1],index=['方差','累计贡献'],columns=[i+1 for i in range(2)])
...
plt.rcParams["font.sans-serif"]="SimHei"#设置字体为这个，显示中文
plt.rcParams["axes.unicode_minus"]=False#坐标轴刻度显示符号
...
pcadata.to_excel(write,sheet_name='PCA结果')
pca_var.to_excel(write,sheet_name='方差结果')
write.save()
