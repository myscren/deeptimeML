...
def train(file_path,models,epochs):
    ...
    predic=data.loc[:,'SIO2':'CS'].values  #待预测数据 input data for prediction
    ### 数据标准化 nomalization
...
#     Loss = np.zeros(shape=(models,epochs))
    ...
    for k in range(models):
        ##训练数据选取 training data preparation
        ...
        datay=train_data['O2']+noise #生成噪声 add uncertainty
        ...
        for x,y in  kl.split(datax):
            ...
            mlp = MLPRegressor(hidden_layer_sizes = (256,128,64,32),activation='tanh',
                               ...
                               random_state=2022,early_stopping=False,tol=0.0001)  #构造神经网络，每层神经元个数为128、64、32共3层。
